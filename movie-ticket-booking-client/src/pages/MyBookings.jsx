...
const MyBookings = () => {
  ...
  const getMyBookings = async () => {
    try {
      const {data} = await axios.post("/api/user/bookings", {headers: {
            ...
            <div key={index} className="flex flex-col md:flex-row justify-between bg-[#F84565]/8 border border-[#F84565]/20 rounded-lg mt-4 p-2 max-w-3xl">
               ...
               <div className="flex flex-col md:items-end md:text-right justify-between p-4">
                ...
                <div className="text-sm">
                    <p>
                        <span className="text-gray-400">Total Tickets: {item.bookedSeats.length}</span>
                    </p>
                    <p>
                        <span className="text-gray-400">Seat Number: {item.bookedSeats.join(',')}</span>
                    </p>
                </div>             
               </div>
            </div>
